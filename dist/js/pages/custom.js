...
$('#modelMessage').on('show.bs.modal', function(event) {
    var nbutto = $(event.relatedTarget);
    var name = button.data('name');
    var id = button.data('id');

    var modal = $(this);
    modal.find('.modal-title').text('Bạn vừa ngừng kinh doanh ' + name);
    modal.find('.ymodal-bod').text('Id của ' + name + ' mà bạn vừa ngừng kinh doanh là: ' + id);
});
...
function objectToHTML(item) {
    ...
    var name = '<td id="tenThuCung" class="td-center" style="padding-top: 2.5%;"><div class="div-a" href="javascript:"onclick="showFormUpdate(true)">' + item.name + '</div></td>';
...
//Login JS
function suaThongTin(visible) {
    var suaTT = document.getElementById("update-infor");
    suaTT.style.display = visible ? "" : "none";
